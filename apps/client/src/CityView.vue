...
<template>
  <section class="city-view" role="dialog" aria-modal="true" :aria-label="`${city.name} Stadtansicht`">
    <header class="city-header"><div><small>Inselstadt</small><h2>{{ city.name }}</h2></div><div class="city-hud"><span>{{ player.gold.toLocaleString('de-DE') }} G</span><span>{{ Object.values(fleet.cargo).reduce((total, amount) => total + amount, 0) }} / {{ fleet.capacity }} t</span></div><button type="button" aria-label="Stadtansicht schließen" @click="$emit('close')">×</button></header>
