...
<template>
  <section class="city-view" role="dialog" aria-modal="true" :aria-label="`${city.name} Stadtansicht`">
    <header class="city-header"><div><small>Inselstadt</small><h2>{{ city.name }}</h2></div><button type="button" aria-label="Stadtansicht schließen" @click="$emit('close')">×</button></header>
    <nav aria-label="Stadtbereiche"><button :class="{ active: tab === 'overview' }" @click="tab = 'overview'">Übersicht</button><button :class="{ active: tab === 'production' }" @click="tab = 'production'">Produktion</button><button :class="{ active: tab === 'market' }" @click="tab = 'market'">Markt</button></nav>
...
.city-header { display: flex; justify-content: space-between; align-items: center; padding: 1rem; background: #dcb580; border-bottom: 4px solid #9b6a3e; }.city-header h2 { margin: .1rem 0 0; }.city-header button { min-width: 2.75rem; min-height: 2.75rem; border: 0; border-radius: .5rem; font-size: 1.5rem; background: #fff7e5; }
