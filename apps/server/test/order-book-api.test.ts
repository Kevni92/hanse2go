...
describe('Alpha 5 orderbook API', () => {
  ...
  it('creates a covered limit order, exposes the book and cancels it by version', async () => {
    ...
    const created = await app.inject({ method: 'POST', url: '/api/cities/lambrecht/market/orders', payload: { goodId: 'wood', side: 'buy', quantityUnits: 100, limitPriceGoldPerTon: 80, idempotencyKey: 'api-buy-1' } });
    expect(created.statusCode).toBe(200);
    expect(created.json()).toMatchObject({ order: { side: 'buy', status: 'open', remainingQuantityUnits: 100, reservedMoney: 8_040 } });

    const replay = await app.inject({ method: 'POST', url: '/api/cities/lambrecht/market/orders', payload: { goodId: 'wood', side: 'buy', quantityUnits: 100, limitPriceGoldPerTon: 80, idempotencyKey: 'api-buy-1' } });
    expect(replay.statusCode).toBe(200);
    expect(replay.json()).toEqual(created.json());
    expect((await app.inject({ method: 'GET', url: '/api/cities/lambrecht/market/wood/order-book' })).json().bids).toMatchObject([{ limitPriceGoldPerTon: 80, quantityUnits: 100 }]);
